...
## TODO Confirm modules have -p support
...
def volley_mr (case_number, root_folder_path, evidence, profile_to_use):
    ...
    vol_command = "vol --profile=" + profile_type + " -f " + quoted_path + " psscan" + " | awk '{print $2" + '","$3",' \
                                                                                                             '"$4",' \
                                                                                                             '"$1",' \
                                                                                                             '"$5",' \
                                                                                                             '"$6",' \
    ...
    for i, pid in enumerate (piddicts):
        ...
        plugins = ['memmap', 'dlllist', 'handles', 'vadinfo', 'malfind', 'impscan', 'apihooks', 'devicetree',
                   'driverscan', 'enumfunc', 'getservicesids', 'getsids', 'idt', 'imageinfo', 'kpcrscan', 'machoinfo',
                   'mbrparser', 'printkey', 'privs', 'psdispscan', 'ssdt', 'thrdscan', 'usnparser', 'vadtree',
                   'vmwareinfo']
        for n, plugin in enumerate (plugins):

            #run plugin
            print ("\n\n" + pid_dir + " Running " + plugin + ".")
            vol_command = "vol --profile=" + profile_type + " -f " + quoted_path + " " + plugin + " -p " + pid[
                'PID'] + " > " + "'" + pid_folder_path + "/" + plugin + ".txt" + "'"
