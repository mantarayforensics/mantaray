...
#INPUT: Filesystem, offset, absolute path to image, logfile location ("NONE" if logging is not required),
# and mount_point location
...
## TODO Replace with Python Library
## TODO create stand alone utility to use this module
...
def mount (value, key, Image, outfile, mount_point):
    ...
    if (value == "ntfs") or (value == "hpfs"):
        ...
        filesystem_mount = value

    #set up mount command
    if (filesystem_mount == "ntfs-3g"):
        mount_command = "mount -t " + filesystem_mount + " -o loop,ro,show_sys_files,streams_interface=windows," \
                                                         "offset=" + str (
            key) + " " + Image + " " + mount_point
    elif (filesystem_mount == "ext"):
        mount_command = "mount -t " + filesystem_mount + " -o loop,ro,noexec,noload,offset=" + str (
            key) + " " + Image + " " + mount_point
        ...
        parted_command = "echo unit B print q | parted " + Image + " | grep " + key + " | grep hfs+ | awk '{print " \
                                                                                      "$4}' | sed s/B//"
        ...
        #set up loopback
        #losetup_command = "losetup --offset " + str(key) + " --sizelimit " + str(partition_size) + " -r /dev/loop7 "
        # + Image
        ...
        mount_command = "mount -t " + filesystem_mount + " -o loop,ro,offset=" + str (
            key) + ",noexec,noatime,nodiratime " + Image + " " + mount_point
    else:
