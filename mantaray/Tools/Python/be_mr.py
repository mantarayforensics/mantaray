...
## TODO Create seperate function for processing bitstream images
...
def process_folder (evidence, folder_path_be, whitelist_location, speed, outfile, keyword_list, cores_to_use):
    ...
    if (whitelist_location != "NONE") and (keyword_list == "NONE"):
        be_command = "bulk_extractor -o " + folder_path_be + ' -w "' + whitelist_location + '" -j ' + str (
            cores_to_use) + " -R " + evidence
        ...
        be_command = "bulk_extractor -o " + folder_path_be + ' -w "' + whitelist_location + '" -F "' + keyword_list +\
                     '" -j ' + str (
            cores_to_use) + " -R " + evidence
    elif (whitelist_location == "NONE") and (keyword_list != "NONE"):
        be_command = "bulk_extractor -o " + folder_path_be + ' -F "' + keyword_list + '" -j ' + str (
            cores_to_use) + " -R " + evidence
...
#at command line enter item_to_process(folder, l01, image), case_name, output_folder, evidence_to_process,
# whitelist_location, speed
...
def be_mr (item_to_process, case_number, folder_path, evidence, whitelist_location, speed, keyword_list):
    ...
    if (item_to_process == "Directory"):
    ...
    elif (item_to_process == "Single File") or (item_to_process == "Memory Image") or (
        item_to_process == "EnCase Logical Evidence File"):

        #set up bulk extractor command
        if (whitelist_location != "NONE") and (keyword_list == "NONE"):
            be_command = "bulk_extractor -o " + folder_path_be + ' -w "' + whitelist_location + '" -j ' + str (
                cores_to_use) + " " + evidence
        elif (whitelist_location == "NONE") and (keyword_list == "NONE"):
            be_command = "bulk_extractor -o " + folder_path_be + " -j " + str (cores_to_use) + " " + evidence
        elif (whitelist_location != "NONE") and (keyword_list != "NONE"):
            be_command = "bulk_extractor -o " + folder_path_be + ' -w "' + whitelist_location + '" -F "' + \
                         keyword_list + '" -j ' + str (
                cores_to_use) + " " + evidence
        elif (whitelist_location == "NONE") and (keyword_list != "NONE"):
            be_command = "bulk_extractor -o " + folder_path_be + ' -F "' + keyword_list + '" -j ' + str (
        ...
        #set up bulk extractor command
        if (whitelist_location != "NONE") and (keyword_list == "NONE"):
            be_command = "bulk_extractor -C 60 -o " + folder_path_be + ' -w "' + whitelist_location + '" -j ' + str (
                cores_to_use) + " " + evidence
        elif (whitelist_location == "NONE") and (keyword_list == "NONE"):
            be_command = "bulk_extractor -C 60 -o " + folder_path_be + " -j " + str (cores_to_use) + " " + evidence
        elif (whitelist_location != "NONE") and (keyword_list != "NONE"):
            be_command = "bulk_extractor -C 60 -o " + folder_path_be + ' -w "' + whitelist_location + '" -F "' + \
                         keyword_list + '" -j ' + str (
                cores_to_use) + " " + evidence
        elif (whitelist_location == "NONE") and (keyword_list != "NONE"):
            be_command = "bulk_extractor -C 60 -o " + folder_path_be + ' -F "' + keyword_list + '" -j ' + str (
                cores_to_use) + " " + evidence
        ...
        #run fiwalk
        fiwalk_command = "fiwalk -x " + evidence + " > " + '"' + folder_path + \
                         '/Bulk_Extractor_Results/fiwalk_output.xml' + '"'
        print ("Running fiwalk: " + fiwalk_command)
        subprocess.call ([fiwalk_command], shell=True)

        #run identify_filenames.py
        identify_filenames_command = "python3 /usr/share/bulk-extractor/python/identify_filenames.py --all " \
                                     "--imagefile " + evidence + " --xmlfile " + '"' + folder_path + \
                                     "/Bulk_Extractor_Results/fiwalk_output.xml" + '" "' + folder_path + \
                                     "/Bulk_Extractor_Results" + '" "' + folder_path + \
                                     "/Bulk_Extractor_Results/annotated_results/" + '"'
