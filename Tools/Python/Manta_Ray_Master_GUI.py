...
def splashscreen():
	...
	gif_path = "/usr/share/mantaray/images/MR_FSISAC.gif"

	intro_splashscreen = buttonbox(msg='', image=gif_path,
					title='MantaRay - github.com/mantarayforensics',
...
if fieldValues: 
	...
	if evidence_type:
		...
		subprocess.call(['zenity --info --title "MantaRay - " '
...
if evidence_type == "Bit-Stream Image":
	batch_mode_enabler = subprocess.check_output(['zenity --list --radiolist --title "MantaRay - github.com/mantarayforensics" --column="Selection" '
...
if evidence_type == "Bit-Stream Image":
	...
	try:
		processing_scripts = subprocess.check_output(['zenity --list --checklist --title "MantaRay -	github.com/mantarayforensics" --column="Selection" '
...
if evidence_type == "EnCase Logical Evidence File":
	...
	try:
		processing_scripts = subprocess.check_output(['zenity --list --checklist --title "MantaRay - github.com/mantarayforensics" '
...
if evidence_type == "Memory Image":
	...
	if re.search('Volatility',processing_scripts):
		...
		try:
			mem_os = subprocess.check_output(['zenity --list --radiolist --title "MantaRay - '
			...
			if mem_os in os_type:
				...
				subprocess.call(['zenity --error --title "MantaRay - " '
		...
		if mem_os == 'Microsoft Windows':
			try:
				hiber_process = subprocess.check_output(['zenity --list --radiolist --title "MantaRay - '
								' github.com/mantarayforensics" '
...
if evidence_type == "Single File":
	...
	try:
		...
		processing_scripts = subprocess.check_output(['zenity --list --checklist --title "MantaRay - '
								' github.com/mantarayforensics" --column="Selection" '
								'--column="Processing Tool" --column="Description" '
								'--separator="," '
								...
								'FALSE "PST Processor" '
								'"Extracts emails (in .eml format) from PST and bucketizes by address specified" '
...
try:
	...
	debug_mode = subprocess.check_output(['zenity --list --radiolist --title "MantaRay -  '
						' github.com/mantarayforensics" '
...
for x in processing_scripts_list:
	...
	if x == 'BulkExtractor':
		...
		try:
			bulkextractor_options = subprocess.check_output(['zenity --list --checklist --title "MantaRay - '
									' github.com/mantarayforensics" '
		...
		try:
			bulkextractor_processor = subprocess.check_output(['zenity --list --radiolist --title "MantaRay - '
									' github.com/mantarayforensics" '
		...
		try:
			...
			foremost_options = subprocess.check_output(['zenity --list --radiolist --title "MantaRay - '
									' github.com/mantarayforensics" '
									'--column="Selection" --column="Processing Option" '
									'--column="Description" --separator="," '
		...
		"""
		try:
			super_timeline_options = subprocess.check_output(['zenity --list --radiolist --title "MantaRay - github.com/mantarayforensics" --column="Selection" --column="Processing Option" --column="Tool Version" --column="Description" --separator="," FALSE "Log2Timeline" "v. 0.65" "Original timeline tool, outputs mactime, bodyfile, and csv format" FALSE "Plaso" "v. 1.1" "Updated timeline tool, outputs a .dmp file for further processing into a csv, raw, or dynamic output." --text="Select Timeline Tool" --width 800 --height 400'], shell=True, universal_newlines=True)
		...
		if(super_timeline_options == "Plaso"):
			...
			try:
				plaso_output_options = subprocess.check_output(['zenity --list --checklist --title "MantaRay - '
										' github.com/mantarayforensics" '
			...
			try:
				plaso_processor = subprocess.check_output(['zenity --list --radiolist --title "MantaRay - '
										' github.com/mantarayforensics" '
										'--column="Selection" --column="Processor Performance" '
										'--column="Description" --separator="," '
		...
		try:
			select_timezone = subprocess.check_output(['zenity --question --title "MantaRay -  '
								' github.com/mantarayforensics" '
			...
			user_defined_timezone = subprocess.check_output(['zenity --list --radiolist --title "MantaRay -  '
									' github.com/mantarayforensics" '
		...
		try:
			run_option = subprocess.check_output(['zenity --list --radiolist --title "MantaRay - '
								' github.com/mantarayforensics" '
		...
		if run_option == "Single Plugin Processing":
			...
			zen_cmd = """zenity --list --checklist --title "MantaRay -        github.com/mantarayforensics" --column="Selection" --column="Plugin" --column="Known OS Support" --column="Description" --separator="," """ # removed  True "Select Custom Profile"
		...
		zen_cmd = """zenity --list --radiolist --title "MantaRay -        github.com/mantarayforensics" --column="Selection" --column="Profile Name" --separator="," """ # removed  True "Select Custom Profile"
...
for evidence_path in evidence_path_list:
	...
	for x in processing_scripts_list:
		...
		elif x == 'PST Processor':
			if(debug_mode == "ON"):
				pst_processor_mr(evidence_type, base_case_number, folder_path, evidence_path.strip())
				gui_outfile.write("PST Processor...".ljust(35) + "completed successfully".ljust(55) + "\n")
			else:
				try:
					pst_processor_mr(evidence_type, base_case_number, folder_path, evidence_path.strip())
					gui_outfile.write("PST Processor...".ljust(35) + "completed successfully".ljust(55) + "\n")
				except:
					print("Call to PST Processor failed")
					gui_outfile.write("PST Processor failed...Please reprocess with  Debug Mode ON - running MantaRay from command line as root\n")
