...
#It reads the partition table and runs Log2timeline (Kristinn Gudjonsson.) , FLS (Brian Carrier) and MACTIME (Brian
# Carrier) against each partition
#The timezone is determined for each partition.  If no timezone is present the program attempts to determine which
...
def split_csv (case_number, temp_time_wc, folder_path, outfile, timeline_type, file_to_split):
    if (timeline_type == "l2t"):
        #word_count_command = "wc -l " + case_number + "_timeline_modules.csv | awk '{print $1}' > /tmp/wc_" +
        # temp_time_wc + ".txt"
        word_count_command = "wc -l " + file_to_split + " | awk '{print $1}' > /tmp/wc_" + temp_time_wc + ".txt"
    elif (timeline_type == "plaso"):
        #word_count_command = "wc -l " + case_number + "_timeline.csv | awk '{print $1}' > /tmp/wc_" + temp_time_wc +
        #  ".txt"
        word_count_command = "wc -l " + file_to_split + " | awk '{print $1}' > /tmp/wc_" + temp_time_wc + ".txt"
            ...
            for root, dirs, files in os.walk (folder_path):
                for filenames in files:
                    if re.search ("xaa", filenames):
                        ...
                        if (timeline_type == "l2t"):
                            sed_command = "sed -i '1i\ Date,Size,Type,Mode,UID,GID,Meta,L2T_Function,File_Name' " + \
                                          "'" + full_path + "'"

                        elif (timeline_type == "plaso"):
                            sed_command = "sed -i '1i\ Date,Time,Timezone,MACB,Source,SourceType,Type,User,Host," \
                                          "Short,Desc,Version,Filename,Inode,Notes,Format,Extra' " + "'" + full_path \
                                          + "'"
...
def get_account_profile_names (account):
    ...
    #now that we have the length of the root string we need to find the offset of the last "/" in the string,
    # then do a substring from that location
...
def get_timezones_non_windows (value, system_hive_path, outfile, mount_point):
    ...
    print ("The timezone for this non-windows drive is: " + str (timezone_final) + "\n\n")
    if (outfile != "NONE"):
...
def get_timezones_windows (value, system_hive_path, outfile):
    ...
    #set up regp command
    regp_command = "perl /usr/share/windows-perl/regp.pl " + system_hive_path + " | grep -a 'StandardName;REG_SZ' > " \
                                                                                "/tmp/regp_output_" + \
                   temp_time_windows + ".txt"
    ...
    for line in fh_regp:
        ...
        if (re.search ('tzres.dll', line.lower ())):
            ...
            regp_command_vista = "perl /usr/share/windows-perl/regp.pl " + system_hive_path + " | grep -a " \
                                                                                              "'TimeZoneKeyName' > " \
                                                                                              "/tmp/regp_output_nonXP_" + temp_time_windows + ".txt"
            ...
            for line in fh1:
                ...
                if (counter_timezone == 0):
                    if (re.search ('TimeZoneKeyName;REG_SZ', line)):
                        ...
                        if (outfile != "NONE"):
                            outfile.write (
                                "The timezone_final for this nonXP partition is: " + str (timezone_final) + "\n")
        else:
...
def log2timeline (timezone_shift, key, folder_path, outfile, mount_point):
    if (timezone_shift == "NONE"):
        ...
        log2timeline_command = "log2timeline_legacy -v -z UTC -o mactime -m Partition_" + key + " -r " + mount_point \
                               + " -log " + "'" + folder_path + "/bodyfile_" + key + "_log2timeline_log.txt" + "'" + \
                               " -d -w " + "'" + folder_path + "/bodyfile_" + key + "'"
        print ("The log2timeline command is: " + log2timeline_command + "\n\n")
        print ("Running Log2timeline against: " + mount_point + "\n\n")
        if (outfile != "NONE"):
...
def plaso (key, folder_path, outfile, Image_Path, timezone, plaso_output_options, case_number, cores_to_use):
    ...
    if (timezone == "NONE"):
        timezone = "UTC"
    plaso_command = "log2timeline.py -o " + key + " -z " + timezone + " -t '" + case_number + "_Partition_" + key + \
                    "_' --workers " + cores_to_use + " --logfile '" + folder_path + "/Timeline_Logs/plaso_logfile" + \
                    key + ".txt' '" + folder_path + "/partition_" + key + ".dmp" + "'" + " " + Image_Path + ""
    #plaso_command = "log2timeline.py -o " + key + " -t '" + case_number + "_Partition_" + key + "_' --workers " +
    # cores_to_use + " --logfile '" + folder_path + "/Timeline_Logs/plaso_logfile" + key + ".txt' '" + folder_path
    # +"/partition_" + key + ".dmp" + "'" +" " + Image_Path + ""
    #else:
    #	plaso_command = "log2timeline.py -o " + key + " -z " + timezone_shift + " -t '" + case_number + "_Partition_"
    # + key + "_' --workers " + cores_to_use + " --logfile '" + folder_path + "/Timeline_Logs/plaso_logfile" + key +
    # ".txt' '" + folder_path +"/partition_" + key + ".dmp" + "'" +" " + Image_Path + ""
    ...
    if run_l2tcsv == "False" and run_sqlite == "False" and run_raw == "False" and run_dynamic == "False" and \
                    run_kibana == "False":
        print ("No output type selected for plaso, saving dump file")

    #set & run pinfo command
    pinfo_cmd = "pinfo.py -v '" + folder_path + "/partition_" + key + ".dmp' > '" + folder_path + "/pinfo_partition_" \
                + key + ".txt'"
    ...
    if run_l2tcsv == "True":
        psort_cmd = "psort.py -o L2tcsv -z " + timezone + " -w '" + folder_path + "/partition_" + key + \
                    "_timeline.csv' '" + folder_path + "/partition_" + key + ".dmp" + "'"
    ...
    if run_sqlite == "True":
        psort_cmd = "psort.py -o Sql4n6 -z " + timezone + " -w '" + folder_path + "/partition_" + key + "_sqlite' '" \
                    + folder_path + "/partition_" + key + ".dmp" + "'"
        print ("The Plaso Psort command is: " + psort_cmd + "\n\n")
        if (outfile != "NONE"):
            outfile.write ("The Plaso Psort command is: " + psort_cmd + "\n\n")
        subprocess.call ([psort_cmd], shell=True)
    ...
    ##################################
    ## If you have followed the guide at https://github.com/mantarayforensics/mantaray/blob/kibana/README.md
    ## Uncommenting the code below will allow the use of Kibana and Elastic Search!
    ##################################
    ...
    if run_dynamic == "True":
        psort_cmd = "psort.py -o Dynamic -z " + timezone + " -w '" + folder_path + "/partition_" + key + \
                    "_dynamic.csv' '" + folder_path + "/partition_" + key + ".dmp" + "'"
        print ("The Plaso Psort command is: " + psort_cmd + "\n\n")
        if (outfile != "NONE"):
            outfile.write ("The Plaso Psort command is: " + psort_cmd + "\n\n")
        subprocess.call ([psort_cmd], shell=True)
    ...
    if run_raw == "True":
        psort_cmd = "psort.py -o Rawpy -z " + timezone + " -w '" + folder_path + "/partition_" + key + "_raw.txt' '" \
                    + folder_path + "/partition_" + key + ".dmp" + "'"
        print ("The Plaso Psort command is: " + psort_cmd + "\n\n")
        if (outfile != "NONE"):
            outfile.write ("The Plaso Psort command is: " + psort_cmd + "\n\n")
...
def fls (key, folder_path, value, outfile, Image_Path):
    #initialize variables
    fls_value = ""
    ...
    fls_command = "fls -o " + str (
        sector) + " -f " + fls_value + " -r -i raw -m Partition_Offset_" + key + " " + Image_Path + ">> " + "'" + \
                  folder_path + "/bodyfile_" + key + "'"

    print ("The fls command is: " + fls_command + "\n\n")
    if (outfile != "NONE"):
...
### REGTIME ##############################################################################
def regtime (system_hive_path, system_hive_regback_path, sam_hive_path, sam_hive_regback_path, security_hive_path,
             security_hive_regback_path, software_hive_path, software_hive_regback_path, folder_path, partition_start,
             outfile):
    print ("Running Regtime.pl against the registry hives\n\n")

    if (system_hive_path != "NONE"):
        #setup system_hive command
        system_hive_command = "perl /usr/share/windows-perl/regtime.pl -m HKLM-SYSTEM_" + partition_start + " -r " + \
                              system_hive_path + " >> " + folder_path + "/bodyfile_" + partition_start
        print ("Running Regtime.pl against: " + system_hive_path)
        print ("The system_hive_command is: " + system_hive_command + "\n\n")
        if (outfile != "NONE"):
    ...
    if (system_hive_regback_path != "NONE"):
        #setup system_hive command
        system_hive_regback_command = "perl /usr/share/windows-perl/regtime.pl -m HKLM-SYSTEM_REGBACK_" + \
                                      partition_start + " -r " + system_hive_regback_path + " >> " + "'" + \
                                      folder_path + "/bodyfile_" + partition_start + "'"
        print ("Running Regtime.pl against: " + system_hive_regback_path)
        print ("The system_hive_regback_command is: " + system_hive_regback_command + "\n\n")
        if (outfile != "NONE"):
    ...
    if (sam_hive_regback_path != "NONE"):
        ...
        sam_hive_regback_command = "perl /usr/share/windows-perl/regtime.pl -m HKLM-SAM_REGBACK_" + partition_start +\
                                   " -r " + sam_hive_regback_path + " >> " + "'" + folder_path + "/bodyfile_" + \
                                   partition_start + "'"
        print ("Running Regtime.pl against: " + sam_hive_regback_path)
        print ("The sam_hive_regback_command is: " + sam_hive_regback_command + "\n\n")
        if (outfile != "NONE"):
    ...
    if (sam_hive_path != "NONE"):
        #setup sam_hive command
        sam_hive_command = "perl /usr/share/windows-perl/regtime.pl -m HKLM-SAM_" + partition_start + " -r " + \
                           sam_hive_path + " >> " + "'" + folder_path + "/bodyfile_" + partition_start + "'"
        print ("Running Regtime.pl against: " + sam_hive_path)
        print ("The sam_hive_command is: " + sam_hive_command + "\n\n")
        if (outfile != "NONE"):
    ...
    if (security_hive_path != "NONE"):
        ...
        security_hive_command = "perl /usr/share/windows-perl/regtime.pl -m HKLM-SECURITY_" + partition_start + " -r " \
                                                                                                                "" + \
                                security_hive_path + " >> " + "'" + folder_path + "/bodyfile_" + partition_start + "'"
        print ("Running Regtime.pl against: " + security_hive_path)
        print ("The security_hive_command is: " + security_hive_command + "\n\n")
        if (outfile != "NONE"):
    ...
    if (security_hive_regback_path != "NONE"):
        #setup security_hive command
        security_hive_regback_command = "perl /usr/share/windows-perl/regtime.pl -m HKLM-SECURITY_REGBACK_" + \
                                        partition_start + " -r " + security_hive_regback_path + " >> " + "'" + \
                                        folder_path + "/bodyfile_" + partition_start + "'"
        print ("Running Regtime.pl against: " + security_hive_regback_path)
        print ("The security_hive_regback_command is: " + security_hive_regback_command + "\n\n")
        if (outfile != "NONE"):
    ...
    if (software_hive_path != "NONE"):
        ...
        software_hive_command = "perl /usr/share/windows-perl/regtime.pl -m HKLM-SOFTWARE_" + partition_start + " -r " \
                                                                                                                "" + \
                                software_hive_path + " >> " + "'" + folder_path + "/bodyfile_" + partition_start + "'"
        print ("Running Regtime.pl against: " + software_hive_path)
        print ("The software_hive_command is: " + software_hive_command + "\n\n")
        if (outfile != "NONE"):
    ...
    if (software_hive_regback_path != "NONE"):
        #setup software_hive command
        software_hive_regback_command = "perl /usr/share/windows-perl/regtime.pl -m HKLM-SOFTWARE_REGBACK_" + \
                                        partition_start + " -r " + software_hive_regback_path + " >> " + "'" + \
                                        folder_path + "/bodyfile_" + partition_start + "'"
        print ("Running Regtime.pl against: " + software_hive_regback_path)
        print ("The software_hive_regback_command is: " + software_hive_regback_command + "\n\n")
        if (outfile != "NONE"):
...
def ntuser_regtime (nt_user_dat, folder_path, key, outfile):
    ...
    for account in nt_user_dat:
        ...
        ntuser_command = "perl /usr/share/windows-perl/regtime.pl -m HTLM-USER-" + profile_name + " -r " + \
                         account_quoted + " >> " + "'" + folder_path + "/bodyfile_" + key + "'"

        print ("the ntuser command is: " + ntuser_command + "\n\n")
        if (outfile != "NONE"):
...
def mactime (timezone, offset, outfile, folder_path):
    if (timezone == "NONE"):
        ...
        mactime_command = "mactime -d -b " + "'" + folder_path + "/bodyfile_" + str (
            offset) + "'" + " -z UTC" + " > " + "'" + folder_path + "/bodyfile_" + str (offset) + "_mactime" + "'"
    else:
        #run mactime for data in this partition
        mactime_command = "mactime -d -b " + "'" + folder_path + "/bodyfile_" + str (
            offset) + "'" + " -z " + timezone + " > " + "'" + folder_path + "/bodyfile_" + str (
            offset) + "_mactime" + "'"
...
def process_folder (folder_to_process, folder_path, outfile, case_number, user_defined_timezone):
    ...
    if (user_defined_timezone == "NONE"):
        ...
        log2timeline_command = "log2timeline_legacy -v -z " + user_defined_timezone + " -o mactime -m Folder -r " + \
                               "'" + folder_to_process + "'" + " -log " + "'" + folder_path + \
                               "/bodyfile_log2timeline_log.txt" + "'" + " -d -w " + "'" + folder_path + \
                               "/bodyfile.txt" + "'"
    print ("The log2timeline command is: " + log2timeline_command + "\n\n")
    print ("Running Log2timeline against: " + folder_to_process + "\n\n")
    subprocess.call ([log2timeline_command], shell=True)
    if (outfile != "NONE"):
        outfile.write ("The log2timeline command is: " + log2timeline_command + "\n\n")
    ...
    if (user_defined_timezone == "NONE"):
        mactime_command = "mactime -d -b " + "'" + folder_path + "/bodyfile.txt" + "'" + "> " + "'" + folder_path + \
                          "/" + case_number + "_timeline.csv" + "'"
    else:
        mactime_command = "mactime -d -z " + user_defined_timezone + " -b " + "'" + folder_path + "/bodyfile.txt" + \
                          "'" + "> " + "'" + folder_path + "/" + case_number + "_timeline.csv" + "'"
...
def plaso_process_folder (Image_Path, folder_path, outfile, case_number, user_defined_timezone, plaso_output_options,
                          cores_to_use):
    ...
    try:
        ...
        if (outfile != "NONE"):
            outfile.write (
                "PLASO DIRECTORY PARSER: Error converting processor cores to string. Reverting to default value of 1 "
                "core for processing.")
        cores_to_use_str = "1"

    #set plaso command
    if (user_defined_timezone == "NONE"):
        plaso_command = "log2timeline.py --workers " + cores_to_use_str + " -t '" + case_number + "' -z UTC -p " \
                                                                                                  "--logfile '" + \
                        folder_path + "/Timeline_Logs/plaso_logfile.txt' '" + folder_path + "/plaso_timeline.dmp" + \
                        "'" + " " + Image_Path + ""
    else:
        plaso_command = "log2timeline.py --workers " + cores_to_use_str + " -t '" + case_number + "' -z " + \
                        user_defined_timezone + " --logfile '" + folder_path + "/Timeline_Logs/plaso_logfile.txt' '" \
                        + folder_path + "/plaso_timeline.dmp" + "'" + " " + Image_Path + ""
    print ("The Plaso command is: " + plaso_command + "\n\n")
    print ("Running Plaso against: " + Image_Path + "\n\n")
    if (outfile != "NONE"):
        outfile.write ("The plaso command is: " + plaso_command + "\n\n")
    ...
    if run_l2tcsv == "True":
        if (user_defined_timezone == "NONE"):
            psort_cmd = "psort.py -o L2tcsv -z UTC" + " -w " + folder_path + "/plaso_timeline.csv " + folder_path + \
                        "/plaso_timeline.dmp"
        else:
            psort_cmd = "psort.py -o L2tcsv -z " + user_defined_timezone + " -w " + folder_path + \
                        "/plaso_timeline.csv " + folder_path + "/plaso_timeline.dmp"
        print ("The Plaso Psort command is: " + psort_cmd + "\n\n")
        if (outfile != "NONE"):
            outfile.write ("The Plaso Psort command is: " + psort_cmd + "\n\n")
        subprocess.call ([psort_cmd], shell=True)
    ...
    if run_sqlite == "True":
        if (user_defined_timezone == "NONE"):
            psort_cmd = "psort.py -o Sql4n6 -z UTC" + " -w " + folder_path + "/plaso_sqlite " + folder_path + \
                        "/plaso_timeline.dmp"
        else:
            psort_cmd = "psort.py -o Sql4n6 -z" + user_defined_timezone + " -w " + folder_path + "/plaso_sqlite " + \
                        folder_path + "/plaso_timeline.dmp"
        print ("The Plaso Psort command is: " + psort_cmd + "\n\n")
        if (outfile != "NONE"):
            outfile.write ("The Plaso Psort command is: " + psort_cmd + "\n\n")
        subprocess.call ([psort_cmd], shell=True)
    ...
    if run_raw == "True":
        if (user_defined_timezone == "NONE"):
            psort_cmd = "psort.py -o Rawpy -z UTC" + " -w " + folder_path + "/plaso_timeline_raw.txt " + folder_path \
                        + "/plaso_timeline.dmp"
        else:
            psort_cmd = "psort.py -o Rawpy -z " + user_defined_timezone + " -w " + folder_path + \
                        "/plaso_timeline_raw.txt " + folder_path + "/plaso_timeline.dmp"
        print ("The Plaso Psort command is: " + psort_cmd + "\n\n")
        if (outfile != "NONE"):
            outfile.write ("The Plaso Psort command is: " + psort_cmd + "\n\n")
        subprocess.call ([psort_cmd], shell=True)
    ...
    if run_dynamic == "True":
        if (user_defined_timezone == "NONE"):
            psort_cmd = "psort.py -o Dynamic -z UTC" + " -w " + folder_path + "/plaso_timeline_dynamic.csv " + \
                        folder_path + "/plaso_timeline.dmp"
        else:
            psort_cmd = "psort.py -o Dynamic -z " + user_defined_timezone + " -w " + folder_path + \
                        "/plaso_timeline_dynamic.csv " + folder_path + "/plaso_timeline.dmp"
        print ("The Plaso Psort command is: " + psort_cmd + "\n\n")
        if (outfile != "NONE"):
            outfile.write ("The Plaso Psort command is: " + psort_cmd + "\n\n")
        subprocess.call ([psort_cmd], shell=True)
...
def GUI_Timeline_mr (item_to_process, case_number, root_folder_path, evidence, user_defined_timezone,
    ...
    if (item_to_process == "Directory"):
        ...
        if run_plaso == "True":
            file_to_split = plaso_process_folder (folder_to_process, folder_path, outfile, case_number,
                                                  user_defined_timezone, plaso_output_options, cores_to_use)
            tool = "plaso"
        ...
        if run_l2t == "True":
            ...
            split_csv (case_number, temp_time_wc, folder_path, outfile, tool, file_to_split)
        if run_plaso == "True":
            file_to_split = plaso_process_folder (file_to_process, folder_path, outfile, case_number,
                                                  user_defined_timezone, plaso_output_options, cores_to_use)
            tool = "plaso"
            if (plaso_output_options == "CSV"):
                split_csv (case_number, temp_time_wc, folder_path, outfile, tool, file_to_split)
            ...
            if (success_code):
                ...
                if (user_defined_timezone == "NONE"):
                    ...
                    if (value == "hfs+") or (value == "hfs"):
                        partition_timezone_final = get_timezones_non_windows (value, system_hive_path, outfile,
                                                                              mount_point)
                        ...
                        if (os.path.isfile (system_hive_path)):
                            ...
                            timezones.append (str (partition_timezone_final))
                            #might need to convert partition_timezone_final to string in the two lines below - more
                            # testing needed
                            print ("Adding timezone: " + str (partition_timezone_final) + " to the timezone list\n")
                            outfile.write (
                                "Adding timezone: " + str (partition_timezone_final) + " to the timezone list\n")
                        else:
        ...
        for key, value in sorted (partition_info_dict.items ()):
            ...
            if (success_code):
                ...
                if run_plaso == "True":
                    key_512 = int (key) / 512
                    key_512 = int (key_512)
                    plaso (str (key_512), folder_path, outfile, Image_Path, timezone_final, plaso_output_options,
                           case_number, cores_to_use)
