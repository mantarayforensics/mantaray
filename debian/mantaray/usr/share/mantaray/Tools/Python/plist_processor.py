...
## TODO Add support for more atifacts in OSX
## TODO Add support for more plists
...
def process_folder (folder_to_process, export_file, outfile, outfile_error, now):
    ...
    #plists_to_process = ['com.apple.airport.preferences.plist', 'com.apple.sidebarlists.plist',
                    ...
                    for plist in plists_to_process:
                        if file_name == plist:
                            ...
                            if (file_format == "xml_plist"):
                                ...
                                #convert binary file
                                plutil_command = "plutil -i " + quoted_abs_file_path + " -o /tmp/binary_plists/" + \
                                                 file_name + "_" + md5 + ".plist"
                                print ("The plutil command is: " + plutil_command)
                                try:
                                    subprocess.call ([plutil_command], shell=True)
                                    outfile.write (
                                        "The converted binary plist is named: /tmp/binary_plists/" + file_name + "_"
                                        + md5 + ".plist\n")
                                except:
                                ...
                                process_converted_binary_plist (
                                    "/tmp/binary_plists/" + file_name + "_" + md5 + ".plist", md5, export_file, outfile,
                                    abs_file_path)
