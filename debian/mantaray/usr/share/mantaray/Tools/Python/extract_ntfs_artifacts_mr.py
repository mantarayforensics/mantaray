        ...
        if (item == "NO"):
            if (re.search ('OrphanFiles', line)):
                ...
                icat_command = "icat -r -i raw -f " + value + " -o " + str (
                    ...
                    key) + "_" + file_type + "_DELETED" + "'"
            else:
                #get user profile name
                icat_command = "icat -r -i raw -f " + value + " -o " + str (
                    key_bytes) + " -b " + block_size + " " + Image_Path + " " + inode_number + " > " + "'" + \
                               folder_path + "/" + inode_number + "_Partition_" + str (
                    key) + "_OVERT_" + file_type + "'"
            ...
            if (re.search ('OrphanFiles', line)):
                ...
                icat_command = "icat -r -i raw -f " + value + " -o " + str (
                    key_bytes) + " -b " + block_size + " " + Image_Path + " " + inode_number + " > " + "'" + \
                               folder_path + "/" + inode_number + "_Partition_" + str (
                    key) + "_DELETED_" + file_type + "_" + item + "'"
            else:
                #get user profile name
                icat_command = "icat -r -i raw -f " + value + " -o " + str (
                    key_bytes) + " -b " + block_size + " " + Image_Path + " " + inode_number + " > " + "'" + \
                               folder_path + "/" + inode_number + "_Partition_" + str (
                    key) + "_" + file_type + "_" + item + "'"
...
def process_overt_deleted_files (value, key, Image_Path, outfile, folder_path, block_size, item, temp_time):
    ...
    fls_command = "fls -Fpr -f ntfs -i raw -o " + str (
        key_bytes) + " " + Image_Path + " | grep -i '\$MFT$' | sed s/:// | sed s/*// > /tmp/fls_output_ntfs_" + \
                  temp_time + ".txt"
    ...
    fls_command = "fls -Fpr -f ntfs -i raw -o " + str (
        key_bytes) + " " + Image_Path + " | grep -i '\$LogFile$' | sed s/:// | sed s/*// > /tmp/fls_output_ntfs_" + \
                  temp_time + ".txt"
    #print ("\nThe fls command is: " + fls_command + "\n")
    ...
    fls_command = "fls -Fpr -f ntfs -i raw -o " + str (
        key_bytes) + " " + Image_Path + " | grep -i '\$UsnJrnl.\$J$' | sed s/:// | sed s/*// > /tmp/fls_output_ntfs_" \
                  + temp_time + ".txt"
    #print ("\nThe fls command is: " + fls_command + "\n")
