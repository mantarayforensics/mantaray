...
#Copyright (C) 2013, 2014 Chapin.Bryce@Mantech.com	        			 #
...
### PROCESS FLS OUTPUT ###### ############################################################
def process_fls_output (value, key, Image_Path, block_size, folder_path, log_folder_path, out_folder, item, searchfile,
                        outfile, temp_file):
        ...
        if (item == "NO"):
            if (re.search ('OrphanFiles', line)):
                ...
                icat_command = "icat -r -i raw -f " + value + " -o " + str (
                    key_bytes) + " -b " + block_size + " " + Image_Path + " " + inode_number + " > " + "'" + \
                               out_folder + "/" + inode_number + "_Partition_" + str (
                    key) + "_" + searchfile + "_DELETED" + "'"
            else:
                #get user profile name
                icat_command = "icat -r -i raw -f " + value + " -o " + str (
                    key_bytes) + " -b " + block_size + " " + Image_Path + " " + inode_number + " > " + "'" + \
                               out_folder + "/" + inode_number + "_Partition_" + str (
                    key) + "_OVERT_" + searchfile + "'"
            ...
            if (re.search ('OrphanFiles', line)):
                ...
                icat_command = "icat -r -i raw -f " + value + " -o " + str (
                    key_bytes) + " -b " + block_size + " " + Image_Path + " " + inode_number + " > " + "'" + \
                               out_folder + "/" + inode_number + "_Partition_" + str (
                    key) + "_DELETED_" + searchfile + "_" + item + "'"
            else:
                #get user profile name
                icat_command = "icat -r -i raw -f " + value + " -o " + str (
                    key_bytes) + " -b " + block_size + " " + Image_Path + " " + inode_number + " > " + "'" + \
                               out_folder + "/" + inode_number + "_Partition_" + str (
                    key) + "_" + searchfile + "_" + item + "'"
...
def process_overt_deleted_files (value, key, Image_Path, outfile, folder_path, log_folder_path, out_folder, block_size,
                                 item, temp_time, searchfile):
    #divide offset by block size so it is in correct format for fls
    key_bytes = int (key) // int (block_size)
    ...
    fls_command = "fls -Fpr -f ntfs -i raw -o " + str (
        key_bytes) + " " + Image_Path + " | grep -i '" + searchfile + "' | sed s/:// | sed s/*// > " + \
                  log_folder_path + "/" + temp_file + ".txt"
    ...
    #process fls output
    process_fls_output (value, key, Image_Path, block_size, folder_path, log_folder_path, out_folder, item, searchfile,
                        outfile, temp_file)
...
def check_for_shadow_volumes (Image_Path, key, block_size, outfile, folder_path, log_folder_path, out_folder, temp_time,
                              searchfile):
...
def mount_shadow_volumes (vssvolume_mnt, outfile, folder_path, log_folder_path, out_folder, key, searchfile):
    ...
    for item in vss_volumes:
        ...
        for key, value in partition_info_dict.items ():
            ...
            process_overt_deleted_files (value, key, vssvolume_mnt + "/" + item, outfile, folder_path, log_folder_path,
                                         out_folder, block_size, item, temp_time, searchfile)
...
def filename_search_mr (item_to_process, case_number, root_folder_path, evidence, searchfile):
    ...
    for key, value in partition_info_dict.items ():
        ...
        if (value == "ntfs") or (value == "fat32"):
            ...
            process_overt_deleted_files (value, key, Image_Path, outfile, folder_path, log_folder_path, out_folder,
                                         block_size, item, temp_time, searchfile)
            vssvolume_mnt = check_for_shadow_volumes (Image_Path, key, block_size, outfile, folder_path,
