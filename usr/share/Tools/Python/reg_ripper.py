...
def get_account_profile_names_usrclass (account):
    #This function takes the absolute path of the location of the USRCLASS.DAT file and returns the profile name
    # where it was found
...
intro_splashscreen = buttonbox (msg='RUN REGISTRY RIPPER',
                                image='/home/sansforensics/Tools/Python/images/cropped-regripper1.GIF',
                                title='REG-RIPPER', choices=('Continue', 'Exit'))
...
ntuser_plugins = (
"ares", "vmplayer", "printers", "mspaper", "streammru", "acmru", "logonusername", "winzip", "appcompatflags",
"controlpanel", "startmenuinternetapps_lm", "recentdocs", "wordwheelquery", "odysseus", "adoberdr", "shellfolders",
"userassist2", "putty", "rootkit_revealer", "startmenuinternetapps_cu", "proxysettings", "privoxy", "autorun",
"muicache", "bitbucket_user", "listsoft", "policies_u", "user_win", "haven_and_hearth", "applets", "typedpaths",
"printermru", "mmc", "tsclient", "vmware_vsphere", "iexplore", "gthist", "yahoo_lm", "logon_xp_run", "autoendtasks",
"ie_main", "environment", "domains", "gtwhitelist", "userlocsvc", "dependency_walker", "typedurls", "nero", "comlg32",
"load", "warcraft3", "comdlg32a", "startpage", "skype", "vista_bitbucket", "aports", "yahoo_cu", "aim", "compdesc",
"fileexts", "mp2", "publishingwizard", "realplayer6", "clampitm", "realvnc", "mndmru", "user_run", "userassist",
"ie_settings", "wallpaper", "win7_ua", "unreadmail", "vncviewer", "clampi", "outlook", "bagtest", "cain", "cpldontload",
"arpcache", "sevenzip", "bagtest2", "winvnc", "winlogon_u", "oisc", "mpmru", "brisv", "officedocs", "winrar", "runmru",
"vista_comdlg32", "ccleaner.pl", "vnchooksapplicationprefs", "compatassist", "javafx", "run", "sysinternals")

##set up plugins for SYSTEM
system_plugins = (
"usbstor3", "usb", "mountdev2", "xpedition", "usbstor2", "eventlog", "eventlogs", "crashdump", "timezone",
"crashcontrol", "svcdll", "dllsearch", "devclass", "shutdown", "rdpport", "hibernate", "pagefile", "network",
"mountdev", "mountdev2", "nic2", "usbstor", "nolmhash", "shares", "services", "ide", "ddm", "stillimage", "imagedev",
"disablelastaccess", "compname", "fw_config", "routes", "svc2", "shutdowncount", "auditfail", "safeboot", "usbdevices",
"svc", "nic", "producttype", "kbdcrash", "legacy", "termserv", "nic_mst2", "timezone2", "spp_clients.pl",
"filesnottosnapshot.pl", "appcompatcache", "diag_sr", "securityproviders", "termcert", "wpdbusenum")
...
software_plugins = (
"appinitdlls", "winver", "secctr", "urlzone", "cmd_shell", "notify", "schedagent", "ie_version", "kb950582", "port_dev",
"sql_lastconnect", "bitbucket", "regback", "bho", "sfc", "banner", "macaddr", "apppaths", "msis", "svchost", "taskman",
"networkuid", "vista_wireless", "installedcomp", "drwatson", "shelloverlay", "winlogon", "ctrlpnl", "virut", "winnt_cv",
"ssid", "shellexec", "win_cv", "renocide", "uninstall", "networkcards", "product", "profilelist", "shellext", "landesk",
"assoc", "specaccts", "userinit", "imagefile", "removdev", "networklist", "clsid", "init_dlls", "codeid", "snapshot",
"defbrowser", "soft_run", "EMDMgmt.pl", "winlivemail.pl", "dfrg", "direct", "run", "tracing", "trappoll", "wbem")
...
for key, value in sorted (partition_info_dict.items ()):
    ...
    if (value == "fat32") or (value == "ntfs"):
        ...
        if (system_hive_path != "NONE"):
            system_hive_md5 = subprocess.check_output (['md5sum ' + system_hive_path + "| awk '{print $1}'"],
                                                       shell=True, universal_newlines=True)
        if (system_hive_regback_path != "NONE"):
            system_hive_regback_md5 = subprocess.check_output (
                ['md5sum ' + system_hive_regback_path + "| awk '{print $1}'"], shell=True, universal_newlines=True)
        if (sam_hive_path != "NONE"):
            sam_hive_md5 = subprocess.check_output (['md5sum ' + sam_hive_path + "| awk '{print $1}'"], shell=True,
                                                    universal_newlines=True)
        if (sam_hive_regback_path != "NONE"):
            sam_hive_regback_md5 = subprocess.check_output (['md5sum ' + sam_hive_regback_path + "| awk '{print $1}'"],
                                                            shell=True, universal_newlines=True)
        if (software_hive_path != "NONE"):
            software_hive_md5 = subprocess.check_output (['md5sum ' + software_hive_path + "| awk '{print $1}'"],
                                                         shell=True, universal_newlines=True)
        if (software_hive_regback_path != "NONE"):
            software_hive_regback_md5 = subprocess.check_output (
                ['md5sum ' + software_hive_regback_path + "| awk '{print $1}'"], shell=True, universal_newlines=True)
        if (security_hive_path != "NONE"):
            security_hive_md5 = subprocess.check_output (['md5sum ' + security_hive_path + "| awk '{print $1}'"],
                                                         shell=True, universal_newlines=True)
        if (security_hive_regback_path != "NONE"):
            security_hive_regback_md5 = subprocess.check_output (
                ['md5sum ' + security_hive_regback_path + "| awk '{print $1}'"], shell=True, universal_newlines=True)
        ...
        if (outfile != "NONE"):
            ...
            for line in nt_user_dat:
                line.strip ()
                user_account_md5 = subprocess.check_output (['md5sum ' + "'" + line + "'" + "| awk '{print $1}'"],
                                                            shell=True, universal_newlines=True)
        ...
        for user_account in nt_user_dat:
            ...
            for plugin in ntuser_plugins:
                ...
                rip_ntuser_command = "perl /usr/share/regripper/rip.pl -r " + quoted_user_account + " -p " + plugin +\
                                     " > " + "'" + folder_path + "/Regripper_Partition_" + str (
                    key) + "/NTUSER_INFO/" + account_name + "/" + plugin + ".txt" + "'"
        ...
        for user_account in usrclass_dat:
            ...
            if not os.path.exists (folder_path + "/Regripper_Partition_" + str (
                    key) + "/USRCLASS_INFO/" + account_name):
            ...
            for plugin in usrclass_plugins:
                print ("Currently running plugin: " + plugin + " against " + account_name + "\n")
                outfile.write ("Currently running plugin: " + plugin + " against " + account_name + "\n")
                rip_usrclass_command = "perl /usr/share/regripper/rip.pl -r " + quoted_user_account + " -p " + plugin \
                                       + " > " + "'" + folder_path + "/Regripper_Partition_" + str (
                    key) + "/USRCLASS_INFO/" + account_name + "/" + plugin + ".txt" + "'"
        ...
        if (system_hive_path != "NONE"):
            ...
            for plugin in system_plugins:
                ...
                outfile.write ("Currently running plugin: " + plugin + " against SYSTEM HIVE with MD5: " + str (
                    system_hive_md5) + "\n")

                rip_system_command = "perl /usr/share/regripper/rip.pl -r " + system_hive_path + " -p " + plugin + " > " + "'" + folder_path + "/Regripper_Partition_" + str (
                    key) + "/SYSTEM/" + plugin + ".txt" + "'"
        ...
        if (system_hive_regback_path != "NONE"):
            ...
            for plugin in system_plugins:
                ...
                outfile.write ("Currently running plugin: " + plugin + " against SYSTEM REGBACK HIVE with MD5: " + str (
                    system_hive_regback_md5) + "\n")

                rip_system_regback_command = "perl /usr/share/regripper/rip.pl -r " + system_hive_regback_path + " -p " + plugin + " > " + "'" + folder_path + "/Regripper_Partition_" + str (
                    key) + "/REGBACK_SYSTEM/" + plugin + ".txt" + "'"
        ...
        if (sam_hive_path != "NONE"):
            ...
            rip_sam_command = "perl /usr/share/regripper/rip.pl -r " + sam_hive_path + " -p samparse > " + "'" + \
                              folder_path + "/Regripper_Partition_" + str (
                key) + "/SAM/samparse.txt" + "'"
        ...
        if (sam_hive_regback_path != "NONE"):
            ...
            outfile.write ("Currently running plugin: samparse against SAM REGBACK HIVE with MD5: " + str (
                sam_hive_regback_md5) + "\n")

            rip_sam_regback_command = "perl /usr/share/regripper/rip.pl -r " + sam_hive_regback_path + " -p samparse " \
                                                                                                       "> " + "'" + \
                                      folder_path + "/Regripper_Partition_" + str (
                key) + "/REGBACK_SAM/samparse.txt" + "'"
        ...
        if (security_hive_path != "NONE"):
            ...
            for plugin in security_plugins:
                ...
                outfile.write ("Currently running plugin: " + plugin + " against SECURITY HIVE with MD5: " + str (
                    security_hive_md5) + "\n")

                rip_security_command = "perl /usr/share/regripper/rip.pl -r " + security_hive_path + " -p " + plugin \
                                       + " > " + "'" + folder_path + "/Regripper_Partition_" + str (
                    key) + "/SECURITY/" + plugin + ".txt" + "'"
        ...
        if (security_hive_regback_path != "NONE"):
            ...
            for plugin in security_plugins:
                ...
                outfile.write (
                    "Currently running plugin: " + plugin + " against SECURITY REGBACK HIVE with MD5: " + str (
                        security_hive_regback_md5) + "\n")

                rip_security_regback_command = "perl /usr/share/regripper/rip.pl -r " + security_hive_regback_path + \
                                               " -p " + plugin + " > " + "'" + folder_path + "/Regripper_Partition_" \
                                               + str (
                    key) + "/REGBACK_SECURITY/" + plugin + ".txt" + "'"
        ...
        if (software_hive_path != "NONE"):
            ...
            for plugin in software_plugins:
                ...
                outfile.write ("Currently running plugin: " + plugin + " against SOFTWARE HIVE with MD5: " + str (
                    software_hive_md5) + "\n")

                rip_software_command = "perl /usr/share/regripper/rip.pl -r " + software_hive_path + " -p " + plugin + " > " + "'" + folder_path + "/Regripper_Partition_" + str (
                    key) + "/SOFTWARE/" + plugin + ".txt" + "'"
        ...
        if (software_hive_regback_path != "NONE"):
            ...
            for plugin in software_plugins:
                ...
                outfile.write (
                    "Currently running plugin: " + plugin + " against SOFTWARE REGBACK HIVE with MD5: " + str (
                        software_hive_regback_md5) + "\n")

                rip_software_regback_command = "perl /usr/share/regripper/rip.pl -r " + software_hive_regback_path + " -p " + plugin + " > " + "'" + folder_path + "/Regripper_Partition_" + str (
                    key) + "/REGBACK_SOFTWARE/" + plugin + ".txt" + "'"
