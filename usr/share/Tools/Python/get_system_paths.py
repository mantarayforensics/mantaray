...
#OUTPUT: LIST containing either "NONE" or absolute path to Registry hive (ex:
# /mnt/windows_mount/WINDOWS/SYSTEM32/config/SYSTEM)
    ...
    if (value == "hfs+"):
        ...
        for root, dirs, files in os.walk (mount_point):
            for filenames in files:
                if (os.path.join (root, filenames).lower () == mount_point + '/windows/system32/config/system'):
                    ...
                    os.path.join (root, filenames).lower () == mount_point + '/windows/system32/config/regback/system'):
        ...
        for root, dirs, files in os.walk (mount_point):
            for filenames in files:
                ...
                elif (os.path.join (root,
                                    filenames).lower () == mount_point + '/windows/system32/config/regback/software'):
                ...
                if (os.path.join (root, filenames).lower () == mount_point + '/windows/system32/config/security'):
                    ...
                    if (outfile != "NONE"):
                        outfile.write ("SECURITY HIVE: " + security_hive_path + "\n")
                                    ...
                                    filenames).lower () == mount_point + '/windows/system32/config/regback/security'):
        ...
        #close print border
        if (prnt == "YES"):
            print ("***********REGISTRY FILE LOCATIONS***********************************", end="\n\n")
        if (outfile != "NONE"):
            outfile.write ("***********REGISTRY FILE LOCATIONS************************************\n\n")
