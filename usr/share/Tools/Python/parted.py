...
#INPUT: This modules takes the logfile location (or NONE if logging is not required) and the absolute path to the
# image file to run parted against
#OUTPUT: This module returns a dictionary containing the partition info for the entire drive (key=offset,
# value=filesystem)
...
def parted (outfile, Image_Path):
    ...
    parted = print_partition + " | parted " + Image_Path
    parted_output = print_partition + " | parted " + Image_Path + " > /tmp/timeline_partition_info_" + temp_time + \
                    ".txt"
    ...
    print ("************** PARTITION INFORMATION *******************************************", end="\n\n")
    if (outfile):
        outfile.write ("************** PARTITION INFORMATION *******************************************\n\n")
