...
def jumplist_mr (item_to_process, case_number, root_folder_path, evidence):
    ...
    for key, value in sorted (partition_info_dict.items ()):
        ...
        if (success_code):
                ...
                for root, dirs, files in os.walk (mount_point):
                    for filenames in files:
                        ...
                        if (fileExtension.lower () == ".automaticdestinations-ms"):
                            ...
                            #jl_command = "perl /usr/share/windows-perl/jl.pl -u " + "'" + profile + "'" + " -f " +
                            # full_path + " >> " + "'" + folder_path + "/jumplist_metadata.txt" + "'"
                            jl_command_tln = "perl /usr/share/windows-perl/jl2.pl " + quoted_full_path + " >> " + "'" \
                                             + folder_path + "/jumplist_metadata_tln.txt" + "'"
            ...
            parse_command = "perl /usr/share/windows-perl/parse.pl -f " + "'" + folder_path + \
                            "/jumplist_metadata_tln.txt" + "'" + "> " + "'" + folder_path + "/jumplist_timeline.txt" \
                            + "'"
    ...
    if (os.path.exists (mount_point + "_ewf")):
        print ("Unmounting mount point for ewf before exiting", end="\n\n")
